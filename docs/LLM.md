...
- **素材**：`<数据目录>/memes/<名称>.png`（`scripts\import-animations.ps1 -All` 导入上游 27 张）；
  配置里的 `memes` 是「名称 → 内容描述」，缺整段时从**内置模板**回落；
- **池**：只认"图真在 + 描述非空"，按名称有序（提示词清单在多次请求间稳定）；
- **碎碎念**：随机抽一张，把描述注入 user 指令（照抄上游措辞），让那句话配合画面；
- **对话**：把整张清单交给模型，它在末尾写 `[图:名称]`；**只在池内命中才采纳**，
  幻觉名字忽略但**绝不吞正文**；记忆里存剥掉标记后的干净正文；
- **渲染**：气泡窗多一个 `<img>`（168×168，`object-fit: contain`），
  **绝对地址由宿主拼**（`pet_protocol::asset_base_url()`）；
- **协议白名单**：素材协议只服务 `webm/` 与 `memes/` 两个子目录（每加一类都要显式登记），
  路径穿越防护不变；
- **诊断**：`<img>` 挂 onload/onerror 回报——配图加载**静默失败**过一次（两个 bug），
  这条日志是"到底渲染出来没有"的唯一证据。

## 5.6 余额 / 用量（已实现）

- 服务商：**只认两家**（同上游）——`deepseek` → `/user/balance`、`opencode-go` → `/zen/go/v1/usage`，
  其余显式 `unsupported`；
- 超时 20s，失败重试 3 次、指数退避 800/1600/3200ms；
- 档位（决定播哪条余额动画）：DeepSeek `100 − min(100, 余额/¥20×100)`、OpenCode `max(三窗口)`，
  再 `100% → 5 / min(4, floor(已用%/20))`——与上游同一套；**下发下标而不是动画名**，
  页面从自己的 `animations.events.balance` 取；
- 入口：托盘「查余额」+ 宠物右键「工具 → 查余额」；展示走气泡（10s）；
- 配置 `llm.balance`：默认关闭、`autoRefresh` 默认关闭（点一次查一次）；
- **已知限制**：密钥库单槽位 → "LLM 一把 key + 余额另一把 key"暂时做不到（下一轮改成映射）。
