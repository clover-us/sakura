...
export interface PetRuntime {
  ...
  interactive: boolean;
  /**
   * **对话输入闸门**：对话输入条正在占用鼠标（拖动/摆弄它），宠物窗口必须整段让开。
   *
   * 为什么把这一步做进宿主、而不是让宠物页自己判断：宠物页能看到的只有
   * "光标在不在我身上"，它无法区分"用户在抓我"和"用户正拖着输入条经过我"。
   * 输入条那一侧（对话窗）知道这件事，于是由它经 `mark_chat_input` 放行闸门，
   * 宿主记账并**下发这个字段**。
   *
   * 前端拿到 `true` 时必须**整段跳过命中判定**（连光标采样的兜底起手也不能做）：
   * 只尊重"窗口吃不吃点击"是不够的——采样通道会绕过窗口样式，宠物照样被拎着走。
   *
   * 闸门带到期时间（宿主侧自愈），所以这个字段可能"自己变回 false"，
   * 前端不要缓存它，每帧读最新的。
   */
  chatInputHeld: boolean;
}
...
export interface CursorSample {
  ...
  primaryDown: boolean;
  /**
   * 主键按下的同时，鼠标是否被**别的进程**的窗口捕获。
   *
   * 由来（用户实测的 bug："在桌面左键框选，鼠标扫过人物就把它带走了"）：
   * 全局采样只知道"按键按着 + 光标在身体上"，分不清"用户抓住了宠物"和"用户正在别的窗口
   * 或桌面上拖选"。真正抓住宠物那一刻，鼠标捕获属于宠物窗口自己（WebView2 会在按下时
   * SetCapture）；在别处拖选时捕获在对方进程手里。宿主查 `GetGUIThreadInfo(0).hwndCapture`
   * 就能把这两种情况分开，前端据此拒绝"外来拖动"的采样起手。
   *
   * 查不到/查询失败时一律为 `false`（= 放行）：这道闸只用来少挡一次误抓，
   * 绝不允许它吞掉正常点击。
   */
