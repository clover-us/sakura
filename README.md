...
- 🖱️ **原生桌面手感**：跟手弹簧拖拽、甩抛物理、逐屏工作区边界（**按身体贴边**：角色贴得住屏幕边）、落地静止、点击 Q 弹、整窗点击穿透（不挡下层窗口，也不会被别人的框选拖走）
...
| 拖拽与抛掷 | 跟手弹簧（临界阻尼、带一点重量）、松手按真实初速甩出、逐屏工作区并集为边界、落地静止在工作区底边；**边界按角色身体算**（不是整块视频画布），所以真的贴得住屏幕边 |
| 点击穿透 | 只有**身体命中区**吃点击；在别处按住左键的拖动（桌面框选、别的窗口里拖选）扫过宠物时**不会被当成抓住宠物**（按下沿 + 鼠标捕获归属两道判据） |
